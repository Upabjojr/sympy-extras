...
| `Reduce` over the complex numbers: Gröbner bases | — | `polys.comprehensive`: comprehensive Gröbner systems (Kapur–Sun–Wang), `resolve(domain=S.Complexes)`; the solutions with the cases of the parameters: `solvers.parametric.parametric_cases` (triangular decomposition with the parameters as the smallest variables), `solve(..., cases=True)` |
